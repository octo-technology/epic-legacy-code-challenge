...
var Sellers = function(db) {
    if(db === undefined) console.log("**** NO DB");
    this.sellersMap = {};
    this.cashHistory = {};
    var self = this;
    this.all = function() {
        var sellers = _.map(this.sellersMap, function (seller) {
            return seller;
        });
    ...
    function add(seller) {
        self.sellersMap[seller.name] = seller;
        self.cashHistory[seller.name] = [];
    }
    function update(seller) {
        var previousCash = self.sellersMap[seller.name].cash;
        self.sellersMap[seller.name] = seller;
        self.sellersMap[seller.name].cash = previousCash;
    }
    Persisting.call(this, db, 'sellers', ['cashHistory', 'sellersMap'])
};

util.inherits(Sellers, Persisting);
...
Sellers.prototype.count = function() {
    return this.all().length;
},

Sellers.prototype.isEmpty = function() {
    return this.count() === 0;
},

Sellers.prototype.updateCash = function(sellerName, amount, currentIteration) {
    var seller = this.get(sellerName);
    seller.cash += parseFloat(amount);
    updateCashHistory(this, seller, currentIteration);
},

Sellers.prototype.setOffline = function(sellerName) {
    this.get(sellerName).online = false;
},

Sellers.prototype.setOnline = function(sellerName) {
